...
async function run(privateKey, ghUser, ghPass) {
  ...
  let result = child.spawnSync(
    `bakerx`,
    `run ansible-srv bionic --ip 192.168.33.10 --sync --memory 3072`.split(" "),
  ...
  result = sshSync(`sudo apt update`, "vagrant@192.168.33.10");
  if (result.error) {
    process.exit(result.status);
  }

  result = sshSync(`sudo apt update`, "vagrant@192.168.33.20");
  if (result.error) {
    process.exit(result.status);
  }
