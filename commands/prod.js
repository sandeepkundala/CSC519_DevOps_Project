...
async function run() {
  ...
  result = sshSync(
    `ansible-playbook ${playbook} -i ${inventoryPath} --vault-password-file /bakerx/cm/vars/pass.txt`,
  ...
  result = sshSync(
    `sudo cp /bakerx/provision/config /home/vagrant/.ssh/config`,
    "vagrant@192.168.33.10"
  );
  if (result.error) {
    process.exit(result.status);
  ...
  result = sshSync(
    `ansible-playbook ${cpPlaybook} -i ${inventoryPath} --vault-password-file /bakerx/cm/vars/pass.txt`,
    "vagrant@192.168.33.10"
  );
  ...
  result = sshSync(
    `ansible-playbook ${mcPlaybook} -i ${awsInventory} --vault-password-file /bakerx/cm/vars/pass.txt`,
    "vagrant@192.168.33.10"
  );
  ...
  result = sshSync(
    `ansible-playbook ${itcPlaybook} -i ${awsInventory} --vault-password-file /bakerx/cm/vars/pass.txt`,
    "vagrant@192.168.33.10"
  );
  ...
  result = sshSync(
    `ansible-playbook ${cbcPlaybook} -i ${awsInventory} --vault-password-file /bakerx/cm/vars/pass.txt`,
    "vagrant@192.168.33.10"
  );
  ...
  let monitor_ip_path =
    "/bakerx/results/192.168.33.20/home/vagrant/monitor_ip.txt";
  ...
  console.log(
    chalk.greenBright("Open browser and go to http://" + monitor_ip + ":8080")
  );
}
